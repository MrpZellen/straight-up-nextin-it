...
export async function GET(context) {
    ...
    const result = await client.db("whereToGo").collection("userInfo").find({"username": uid}).toArray()
    console.log(result)
...
export async function POST(request, context) {
    ...
    try {
      ...
      const overlayPath = `./public/locations/${body.locationType.replaceAll(' ', '-')}-location.png`;
      ...
      .png()
      .toBuffer();
      const fileName = "/userPhotos/" + body.locationType.replaceAll(' ', '-') +"/" + body.imageString + ".png";
      fs.writeFileSync(`./public/${fileName}`, mergedBuffer);
      console.log("File saved successfully");
      //first, check if the user has already submitted a location
      const existingUser = await client.db("whereToGo").collection("userInfo").findOneAndUpdate({
        userID: 0, 
      }, {
        $push: { userPhotos: {
